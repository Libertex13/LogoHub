...
export default function Icon() {
  ...
  const { saveImage, loadingSave } = useAuth();
      ...
      {imageUrl && (
        <div className="relative flex justify-center border min-h-full border-indigo-600 rounded-md p-4">
          ...
          {loadingSave ? (
            <button
              type="button"
              className="absolute bottom-0 right-0 mb-6 mr-6 items-center px-4 py-2 inline-flex font-semibold leading-6 text-sm shadow rounded-md text-white bg-indigo-500 hover:bg-indigo-400 transition ease-in-out duration-150 "
              disabled
            >
              <Spinner />
              Saving...
